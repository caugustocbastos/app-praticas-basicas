...
class PraticasBasicasSupabaseUser extends BaseAuthUser {
  PraticasBasicasSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> praticasBasicasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PraticasBasicasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
